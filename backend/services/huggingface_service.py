...
class HuggingFaceService:
    ...
    def _transform_dataset_to_response(
        self, dataset, meta_info: Optional[Dict] = None, actual_size: Optional[int] = None
    ) -> Dict[str, Any]:
    ...
    def _extract_dataset_metadata(self, dataset) -> Dict[str, Any]:
        ...
        return {
            ...
            "taskCategories": tag_metadata["task_categories"]
            if tag_metadata["task_categories"]
            else None,
                ...
                details.update(
                    {
                        ...
                        "videoKeys": [
                            k
                            for k, v in meta_info.get("features", {}).items()
    ...
    def get_episode_data(self, repo_id: str, episode_id: int) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            video_urls = []
            video_keys = [
                k for k, v in meta_info.get("features", {}).items() if v.get("dtype") == "video"
            ]
            ...
            for video_key in video_keys:
                ...
                if "video_path" in meta_info:
                    ...
                    video_url = video_path.format(
                        episode_chunk=chunk, video_key=video_key, episode_index=episode_id
                    )
                    full_url = f"https://huggingface.co/datasets/{repo_id}/resolve/main/{video_url}"
                    video_urls.append({"camera": video_key, "url": full_url})
            ...
            return {
                ...
                "cameras": [
                    {"id": vk, "name": vk.replace("observation.images.", "")} for vk in video_keys
                ],
            }
    ...
    def _get_episode_telemetry(
        self, repo_id: str, episode_id: int, meta_info: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        ...
        try:
            if "data_path" in meta_info:
                ...
                chunk = episode_id // meta_info.get("chunks_size", 1000)
                parquet_path = data_path.format(episode_chunk=chunk, episode_index=episode_id)
                parquet_url = (
                    f"https://huggingface.co/datasets/{repo_id}/resolve/main/{parquet_path}"
                )

                # Read parquet file with pandas
                df = pd.read_parquet(parquet_url, storage_options={"headers": self.headers})
