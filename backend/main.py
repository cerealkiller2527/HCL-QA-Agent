...
async def check_rate_limit(request: Request):
    ...
    if not rate_limiter.check_rate_limit(client_id):
        ...
        raise HTTPException(
            status_code=429, detail="Too many requests. Please wait a moment before trying again."
        )


# Dependency to get HuggingFace service
def get_hf_service(authorization: Optional[str] = Header(None)) -> HuggingFaceService:
...
def read_root():
    return ApiInfo(message="LeRobot Dataset API", version="0.1.0", status="running")
...
def get_user_info(hf_service: HuggingFaceService = Depends(get_hf_service)):
    ...
    try:
        ...
        if "429" in str(e) or "Too Many Requests" in str(e):
            raise HTTPException(
                status_code=429, detail="HuggingFace API rate limit exceeded. Please wait a moment."
            )
...
def list_datasets(
    hf_service: HuggingFaceService = Depends(get_hf_service), limit: Optional[int] = None
):
...
@app.get(
    "/api/v1/datasets/{owner}/{dataset_name}/episodes/{episode_id}",
    response_model=EpisodeDataResponse,
)
...
def get_dataset_episodes(
    owner: str, dataset_name: str, hf_service: HuggingFaceService = Depends(get_hf_service)
):
...
def delete_dataset(
    ...
    try:
        ...
        if "404" in str(e) or "Not Found" in str(e):
            ...
            raise HTTPException(
                status_code=403, detail="You don't have permission to delete this dataset"
            )
        raise HTTPException(status_code=500, detail=str(e))
